...
public class FluxTest {
    ...
    public void fluxSubscriber() {
        Flux<String> fluxString = Flux.just("Rodrigo", "Eduardo", "BLOB")
                .log();

        fluxString.subscribe(string -> log.info("String: {}", string));

        log.info("---------------------------------------------");

        StepVerifier.create(fluxString)
                .expectNext("Rodrigo", "Eduardo", "BLOB")
                .verifyComplete();
    }
    ...
    public void fluxSubscriberNumbersError() {
        ...
        flux.subscribe(i -> log.info("Number: {}", i), Throwable::printStackTrace,
                () -> log.info("TUDO DI BOA!"), s -> s.request(3));
